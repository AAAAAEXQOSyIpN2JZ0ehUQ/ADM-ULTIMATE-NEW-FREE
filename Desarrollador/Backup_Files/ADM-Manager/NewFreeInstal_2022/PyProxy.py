...
def acceptlist():
    IP = str(subprocess.check_output("wget -qO- ipv4.icanhazip.com",shell=True))
    REP = "bn\,'"
    for i in range(0,len(REP)):
        IP=IP.replace(REP[i],"")
    ACCEPTS = []
    ACCEPTS.append(IP)
    ACCEPTS.append('localhost')
    ACCEPTS.append('127.0.0.1')
    return ACCEPTS
...
class SimpleHTTPProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_CONNECT(self):
        req = self
        reqbody = None
        hostip = re.sub(r':(\w*)(.\w{2,3})', '', req.path)
        req.path = "https://%s/" % re.sub(r':(\w*)(.\w{2,3})', '', req.path)
        ...
        try:
            ...
            return
        self.send_response(200, 'Connection Stabilished')
...
def main(HandlerClass=SimpleHTTPProxyHandler, ServerClass=ThreadingHTTPServer, protocol="HTTP/1.1"):
    server_address = ('', port)
